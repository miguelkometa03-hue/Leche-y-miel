...
export default function PlannerScreen() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case "pendiente": return { color: C.warning, label: "Pendiente", bg: C.warningMuted };
      case "en_proceso": return { color: C.water, label: "En proceso", bg: C.waterMuted };
      case "completado": return { color: C.success, label: "Completado", bg: C.successMuted };
      default: return { color: C.textMuted, label: status, bg: C.backgroundTertiary };
    }
  };
      ...
      >
                ...
                >
                  {f.imageUrl ? (
                    ...
                    <View style={styles.formulaPickerImgFallback}>
                      {f.area === "panaderia" ? (
                        <Wheat size={14} color={C.primary} />
                      ) : (
                        <CakeSlice size={14} color={C.gold} />
                      )}
                    </View>
                  )}
        ...
        {dayItems.length > 0 && (
          <View style={styles.summaryBar}>
            ...
            <View style={styles.summaryItem}>
              <Scale size={14} color={C.textSecondary} />
                    ...
                    {formula?.imageUrl ? (
                      ...
                      <View style={styles.planCardImgFallback}>
                        {item.area === "panaderia" ? (
                          ...
                          <CakeSlice size={18} color={C.gold} />
                        )}
                      </View>
                    )}
